...
void Widget::Init(InitParams params) {
  ...
  if (params.dpi_awareness) {
    ...
    SDL_SetWindowSize(window_, static_cast<float>(params.size.x) * dpi,
                      static_cast<float>(params.size.y) * dpi);
  ...
  if (event->type == SDL_EVENT_KEY_DOWN) {
    if (event->key.windowID == self->window_id_) {
      ...
      if (event->key.scancode == SDL_SCANCODE_RETURN &&
        ...
        const bool fullscreen_state = self->IsFullscreen();
